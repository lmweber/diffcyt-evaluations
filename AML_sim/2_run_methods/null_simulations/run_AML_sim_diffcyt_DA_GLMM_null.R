...
out_diffcyt_DA_GLMM_null  <- 
  out_clusters_diffcyt_DA_GLMM_null <- 
  out_objects_diffcyt_DA_GLMM_null <- 
  runtime_diffcyt_DA_GLMM_null <- vector("list", length(thresholds))
names(out_diffcyt_DA_GLMM_null) <- 
  names(out_clusters_diffcyt_DA_GLMM_null) <- 
  names(out_objects_diffcyt_DA_GLMM_null) <- 
  names(runtime_diffcyt_DA_GLMM_null) <- thresholds
...
for (th in 1:length(thresholds)) {
  ...
  # lists to store objects
  out_diffcyt_DA_GLMM_null[[th]]  <- 
    out_clusters_diffcyt_DA_GLMM_null[[th]] <- 
    out_objects_diffcyt_DA_GLMM_null[[th]] <- 
    runtime_diffcyt_DA_GLMM_null[[th]] <- vector("list", length(seed_names))
  names(out_diffcyt_DA_GLMM_null[[th]]) <- 
    names(out_clusters_diffcyt_DA_GLMM_null[[th]]) <- 
    names(out_objects_diffcyt_DA_GLMM_null[[th]]) <- 
    names(runtime_diffcyt_DA_GLMM_null[[th]]) <- seed_names
  ...
  for (s in 1:length(seed_names)) {
    ...
    out_objects_diffcyt_DA_GLMM_null[[th]][[s]] <- list(
      d_se = d_se, 
      d_counts = d_counts, 
      d_medians = d_medians, 
    ...
    runtime_diffcyt_DA_GLMM_null[[th]][[s]] <- runtime_total
    ...
    out_clusters_diffcyt_DA_GLMM_null[[th]][[s]] <- res_clusters
    ...
    # store results
    out_diffcyt_DA_GLMM_null[[th]][[s]] <- res
...
save(out_diffcyt_DA_GLMM_null, runtime_diffcyt_DA_GLMM_null, 
     file = file.path(DIR_RDATA, "outputs_AML_sim_diffcyt_DA_GLMM_null.RData"))

save(out_clusters_diffcyt_DA_GLMM_null, 
     file = file.path(DIR_RDATA, "out_clusters_AML_sim_diffcyt_DA_GLMM_null.RData"))

save(out_objects_diffcyt_DA_GLMM_null, 
     file = file.path(DIR_RDATA, "out_objects_AML_sim_diffcyt_DA_GLMM_null.RData"))
...
sink(file.path(DIR_SESSION_INFO, "session_info_AML_sim_diffcyt_DA_GLMM_null.txt"))
sessionInfo()
sink()
