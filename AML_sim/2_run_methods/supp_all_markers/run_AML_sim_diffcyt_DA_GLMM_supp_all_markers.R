...
for (th in 1:length(thresholds)) {
  ...
  out_diffcyt_DA_GLMM_supp_all_markers[[th]] <- runtime_diffcyt_DA_GLMM_supp_all_markers[[th]] <- vector("list", length(cond_names))
  names(out_diffcyt_DA_GLMM_supp_all_markers[[th]]) <- names(runtime_diffcyt_DA_GLMM_supp_all_markers[[th]]) <- cond_names
  ...
  for (j in 1:length(cond_names)) {
    ...
    runtime_diffcyt_DA_GLMM_supp_all_markers[[th]][[j]] <- runtime_total
    ...
    # store results
    out_diffcyt_DA_GLMM_supp_all_markers[[th]][[j]] <- res
...
save(out_diffcyt_DA_GLMM_supp_all_markers, runtime_diffcyt_DA_GLMM_supp_all_markers, 
     file = file.path(DIR_RDATA, "outputs_AML_sim_diffcyt_DA_GLMM_supp_all_markers.RData"))
...
sink(file.path(DIR_SESSION_INFO, "session_info_AML_sim_diffcyt_DA_GLMM_supp_all_markers.txt"))
sessionInfo()
sink()
