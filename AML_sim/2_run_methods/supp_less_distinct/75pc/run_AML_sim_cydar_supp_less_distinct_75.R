...
out_cydar_supp_less_distinct_75 <- runtime_cydar_supp_less_distinct_75 <- vector("list", length(thresholds))
names(out_cydar_supp_less_distinct_75) <- names(runtime_cydar_supp_less_distinct_75) <- thresholds
...
for (th in 1:length(thresholds)) {
  ...
  out_cydar_supp_less_distinct_75[[th]] <- runtime_cydar_supp_less_distinct_75[[th]] <- vector("list", length(cond_names))
  names(out_cydar_supp_less_distinct_75[[th]]) <- names(runtime_cydar_supp_less_distinct_75[[th]]) <- cond_names
  ...
  for (j in 1:length(cond_names)) {
    ...
    runtime_cydar_supp_less_distinct_75[[th]][[j]] <- runtime_total
    ...
    # store results
    out_cydar_supp_less_distinct_75[[th]][[j]] <- res
...
save(out_cydar_supp_less_distinct_75, runtime_cydar_supp_less_distinct_75, 
     file = file.path(DIR_RDATA, "outputs_AML_sim_cydar_supp_less_distinct_75.RData"))
...
sink(file.path(DIR_SESSION_INFO, "session_info_AML_sim_cydar_supp_less_distinct_75.txt"))
sessionInfo()
sink()
