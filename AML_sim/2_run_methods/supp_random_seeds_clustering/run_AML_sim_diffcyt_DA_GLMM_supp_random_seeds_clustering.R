...
out_diffcyt_DA_GLMM_supp_random_seeds_clustering <- runtime_diffcyt_DA_GLMM_supp_random_seeds_clustering <- 
  out_clusters_diffcyt_DA_GLMM_supp_random_seeds_clustering <- out_objects_diffcyt_DA_GLMM_supp_random_seeds_clustering <- 
  vector("list", length(seeds))
names(out_diffcyt_DA_GLMM_supp_random_seeds_clustering) <- names(runtime_diffcyt_DA_GLMM_supp_random_seeds_clustering) <- 
  names(out_clusters_diffcyt_DA_GLMM_supp_random_seeds_clustering) <- names(out_objects_diffcyt_DA_GLMM_supp_random_seeds_clustering) <- 
  seeds
...
for (s in 1:length(seeds)) {
  ...
  out_diffcyt_DA_GLMM_supp_random_seeds_clustering[[s]] <- runtime_diffcyt_DA_GLMM_supp_random_seeds_clustering[[s]] <- 
    out_clusters_diffcyt_DA_GLMM_supp_random_seeds_clustering[[s]] <- out_objects_diffcyt_DA_GLMM_supp_random_seeds_clustering[[s]] <- 
    vector("list", length(thresholds))
  names(out_diffcyt_DA_GLMM_supp_random_seeds_clustering[[s]]) <- names(runtime_diffcyt_DA_GLMM_supp_random_seeds_clustering[[s]]) <- 
    names(out_clusters_diffcyt_DA_GLMM_supp_random_seeds_clustering[[s]]) <- names(out_objects_diffcyt_DA_GLMM_supp_random_seeds_clustering[[s]]) <- 
    thresholds
  ...
  for (th in 1:length(thresholds)) {
    ...
    out_objects_diffcyt_DA_GLMM_supp_random_seeds_clustering[[s]][[th]] <- list(
      d_se = d_se, 
      d_counts = d_counts, 
      d_medians = d_medians, 
    ...
    out_diffcyt_DA_GLMM_supp_random_seeds_clustering[[s]][[th]] <- runtime_diffcyt_DA_GLMM_supp_random_seeds_clustering[[s]][[th]] <- 
      out_clusters_diffcyt_DA_GLMM_supp_random_seeds_clustering[[s]][[th]] <- vector("list", length(cond_names))
    names(out_diffcyt_DA_GLMM_supp_random_seeds_clustering[[s]][[th]]) <- names(runtime_diffcyt_DA_GLMM_supp_random_seeds_clustering[[s]][[th]]) <- 
      names(out_clusters_diffcyt_DA_GLMM_supp_random_seeds_clustering[[s]][[th]]) <- cond_names
    ...
    for (j in 1:length(cond_names)) {
      ...
      runtime_diffcyt_DA_GLMM_supp_random_seeds_clustering[[s]][[th]][[j]] <- runtime_total
      ...
      out_clusters_diffcyt_DA_GLMM_supp_random_seeds_clustering[[s]][[th]][[j]] <- res_clusters
      ...
      # store results
      out_diffcyt_DA_GLMM_supp_random_seeds_clustering[[s]][[th]][[j]] <- res
...
save(out_diffcyt_DA_GLMM_supp_random_seeds_clustering, runtime_diffcyt_DA_GLMM_supp_random_seeds_clustering, 
     file = file.path(DIR_RDATA, "outputs_AML_sim_diffcyt_DA_GLMM_supp_random_seeds_clustering.RData"))

save(out_clusters_diffcyt_DA_GLMM_supp_random_seeds_clustering, 
     file = file.path(DIR_RDATA, "out_clusters_AML_sim_diffcyt_DA_GLMM_supp_random_seeds_clustering.RData"))

save(out_objects_diffcyt_DA_GLMM_supp_random_seeds_clustering, 
     file = file.path(DIR_RDATA, "out_objects_AML_sim_diffcyt_DA_GLMM_supp_random_seeds_clustering.RData"))
...
sink(file.path(DIR_SESSION_INFO, "session_info_AML_sim_diffcyt_DA_GLMM_supp_random_seeds_clustering.txt"))
sessionInfo()
sink()
