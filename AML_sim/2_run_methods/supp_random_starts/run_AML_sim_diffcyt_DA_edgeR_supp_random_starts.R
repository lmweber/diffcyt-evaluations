...
DIR_RDATA <- "../../../../RData/AML_sim/supp_random_starts"
DIR_SESSION_INFO <- "../../../../session_info/AML_sim/supp_random_starts"
...
# seeds for random starts for clustering
seeds <- c(101, 102, 103)
...
out_diffcyt_DA_edgeR_supp_random_starts <- runtime_diffcyt_DA_edgeR_supp_random_starts <- vector("list", length(seeds))
names(out_diffcyt_DA_edgeR_supp_random_starts) <- names(runtime_diffcyt_DA_edgeR_supp_random_starts) <- seeds
...
for (s in 1:length(seeds)) {
  ...
  out_diffcyt_DA_edgeR_supp_random_starts[[s]] <- runtime_diffcyt_DA_edgeR_supp_random_starts[[s]] <- vector("list", length(thresholds))
  names(out_diffcyt_DA_edgeR_supp_random_starts[[s]]) <- names(runtime_diffcyt_DA_edgeR_supp_random_starts[[s]]) <- thresholds
  ...
  for (th in 1:length(thresholds)) {
    ...
    out_diffcyt_DA_edgeR_supp_random_starts[[s]][[th]] <- runtime_diffcyt_DA_edgeR_supp_random_starts[[s]][[th]] <- vector("list", length(cond_names))
    names(out_diffcyt_DA_edgeR_supp_random_starts[[s]][[th]]) <- names(runtime_diffcyt_DA_edgeR_supp_random_starts[[s]][[th]]) <- cond_names
    ...
    for (j in 1:length(cond_names)) {
      ...
      runtime_diffcyt_DA_edgeR_supp_random_starts[[s]][[th]][[j]] <- runtime_total
      ...
      # store results
      out_diffcyt_DA_edgeR_supp_random_starts[[s]][[th]][[j]] <- res
...
save(out_diffcyt_DA_edgeR_supp_random_starts, runtime_diffcyt_DA_edgeR_supp_random_starts, 
     file = file.path(DIR_RDATA, "outputs_AML_sim_diffcyt_DA_edgeR_supp_random_starts.RData"))
...
sink(file.path(DIR_SESSION_INFO, "session_info_AML_sim_diffcyt_DA_edgeR_supp_random_starts.txt"))
sessionInfo()
sink()
