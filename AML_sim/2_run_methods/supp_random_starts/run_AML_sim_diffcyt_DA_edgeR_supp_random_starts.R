...
out_diffcyt_DA_edgeR_supp_random_starts <- vector("list", length(seeds))
names(out_diffcyt_DA_edgeR_supp_random_starts) <- seeds
...
for (s in 1:length(seeds)) {
  ...
  out_diffcyt_DA_edgeR_supp_random_starts[[s]] <- vector("list", length(thresholds))
  names(out_diffcyt_DA_edgeR_supp_random_starts[[s]]) <- thresholds
  ...
  for (th in 1:length(thresholds)) {
    ...
    out_diffcyt_DA_edgeR_supp_random_starts[[s]][[th]] <- vector("list", length(cond_names))
    names(out_diffcyt_DA_edgeR_supp_random_starts[[s]][[th]]) <- cond_names
...
save(out_diffcyt_DA_edgeR_supp_random_starts, 
