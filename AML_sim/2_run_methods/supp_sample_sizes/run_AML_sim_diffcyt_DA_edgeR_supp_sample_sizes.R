...
contrasts_list <- list(CN = c(0, 1, 0, 0), CBF = c(0, 0, 1, 0))

# define sample sizes: 2 vs. 2
samples_keep <- rep(c(rep(TRUE, 2), rep(FALSE, 3)), 3)

# lists to store objects and runtime
out_diffcyt_DA_edgeR_supp_sample_sizes <- runtime_diffcyt_DA_edgeR_supp_sample_sizes <- 
  out_clusters_diffcyt_DA_edgeR_supp_sample_sizes <- out_objects_diffcyt_DA_edgeR_supp_sample_sizes <- 
  vector("list", length(thresholds))
names(out_diffcyt_DA_edgeR_supp_sample_sizes) <- names(runtime_diffcyt_DA_edgeR_supp_sample_sizes) <- 
  names(out_clusters_diffcyt_DA_edgeR_supp_sample_sizes) <- names(out_objects_diffcyt_DA_edgeR_supp_sample_sizes) <- 
  thresholds
...
for (th in 1:length(thresholds)) {
  ...
  out_objects_diffcyt_DA_edgeR_supp_sample_sizes[[th]] <- list(
    d_se = d_se, 
    d_counts = d_counts, 
    d_medians = d_medians, 
  ...
  out_diffcyt_DA_edgeR_supp_sample_sizes[[th]] <- runtime_diffcyt_DA_edgeR_supp_sample_sizes[[th]] <- 
    out_clusters_diffcyt_DA_edgeR_supp_sample_sizes[[th]] <- vector("list", length(cond_names))
  names(out_diffcyt_DA_edgeR_supp_sample_sizes[[th]]) <- names(runtime_diffcyt_DA_edgeR_supp_sample_sizes[[th]]) <- 
    names(out_clusters_diffcyt_DA_edgeR_supp_sample_sizes[[th]]) <- cond_names
  ...
  for (j in 1:length(cond_names)) {
    ...
    runtime_diffcyt_DA_edgeR_supp_sample_sizes[[th]][[j]] <- runtime_total
    ...
    out_clusters_diffcyt_DA_edgeR_supp_sample_sizes[[th]][[j]] <- res_clusters
    ...
    # store results
    out_diffcyt_DA_edgeR_supp_sample_sizes[[th]][[j]] <- res
...
save(out_diffcyt_DA_edgeR_supp_sample_sizes, runtime_diffcyt_DA_edgeR_supp_sample_sizes, 
     file = file.path(DIR_RDATA, "outputs_AML_sim_diffcyt_DA_edgeR_supp_sample_sizes.RData"))

save(out_clusters_diffcyt_DA_edgeR_supp_sample_sizes, 
     file = file.path(DIR_RDATA, "out_clusters_AML_sim_diffcyt_DA_edgeR_supp_sample_sizes.RData"))

save(out_objects_diffcyt_DA_edgeR_supp_sample_sizes, 
     file = file.path(DIR_RDATA, "out_objects_AML_sim_diffcyt_DA_edgeR_supp_sample_sizes.RData"))
...
sink(file.path(DIR_SESSION_INFO, "session_info_AML_sim_diffcyt_DA_edgeR_supp_sample_sizes.txt"))
sessionInfo()
sink()
