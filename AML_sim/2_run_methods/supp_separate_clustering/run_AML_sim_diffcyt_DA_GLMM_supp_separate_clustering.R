...
out_diffcyt_DA_GLMM_supp_separate_clustering <- runtime_diffcyt_DA_GLMM_supp_separate_clustering <- 
  out_clusters_diffcyt_DA_GLMM_supp_separate_clustering <- out_objects_diffcyt_DA_GLMM_supp_separate_clustering <- 
  vector("list", length(thresholds))
names(out_diffcyt_DA_GLMM_supp_separate_clustering) <- names(runtime_diffcyt_DA_GLMM_supp_separate_clustering) <- 
  names(out_clusters_diffcyt_DA_GLMM_supp_separate_clustering) <- names(out_objects_diffcyt_DA_GLMM_supp_separate_clustering) <- 
  thresholds
...
for (th in 1:length(thresholds)) {
  ...
  out_diffcyt_DA_GLMM_supp_separate_clustering[[th]] <- runtime_diffcyt_DA_GLMM_supp_separate_clustering[[th]] <- 
    out_clusters_diffcyt_DA_GLMM_supp_separate_clustering[[th]] <- out_objects_diffcyt_DA_GLMM_supp_separate_clustering[[th]] <- 
    vector("list", length(cond_names))
  names(out_diffcyt_DA_GLMM_supp_separate_clustering[[th]]) <- names(runtime_diffcyt_DA_GLMM_supp_separate_clustering[[th]]) <- 
    names(out_clusters_diffcyt_DA_GLMM_supp_separate_clustering[[th]]) <- names(out_objects_diffcyt_DA_GLMM_supp_separate_clustering[[th]]) <- 
    cond_names
  ...
  for (j in 1:length(cond_names)) {
    ...
    out_objects_diffcyt_DA_GLMM_supp_separate_clustering[[th]][[j]] <- list(
      d_se = d_se, 
      d_counts = d_counts, 
      d_medians = d_medians, 
    ...
    runtime_j <- system.time({
      ...
      formula <- createFormula(experiment_info_sub, cols_fixed = 1, cols_random = 2:3)
      formula
    ...
    out_clusters_diffcyt_DA_GLMM_supp_separate_clustering[[th]][[j]] <- res_clusters
...
save(out_clusters_diffcyt_DA_GLMM_supp_separate_clustering, 
     file = file.path(DIR_RDATA, "out_clusters_AML_sim_diffcyt_DA_GLMM_supp_separate_clustering.RData"))

save(out_objects_diffcyt_DA_GLMM_supp_separate_clustering, 
     file = file.path(DIR_RDATA, "out_objects_AML_sim_diffcyt_DA_GLMM_supp_separate_clustering.RData"))
