...
# path to save plots
DIR_PLOTS <- "../../../../plots/AML_sim/comparisons_CellCnn"
...
# copy automatically generated CellCnn plots (for one threshold and condition only)

plots_CellCnn <- list.files(file.path(DIR_CELLCNN_FILES, "5pc", "CN", "out_CellCnn", "plots", "training_plots"), 
...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    # color scheme
    colors <- c("darkblue", "dodgerblue", "darkslategray3", "darkorange1")
    ...
    fn <- file.path(DIR_PLOTS, "panels", paste0("results_AML_sim_comparisons_CellCnn_main_ROC_", thresholds[th], "_", cond_names[j], ".pdf"))
    ...
    # save individual panel plot
    fn <- file.path(DIR_PLOTS, "panels", paste0("results_AML_sim_comparisons_CellCnn_main_TPRFDR_", thresholds[th], "_", cond_names[j], ".pdf"))
    ggsave(fn, width = 4.75, height = 3.5)
    ...
    fn <- file.path(DIR_PLOTS, paste0("results_AML_sim_comparisons_CellCnn_main_performance_", thresholds[th], "_", cond_names[j], ".pdf"))
...
for (j in 1:length(cond_names)) {
  ...
  # save combined plot
  fn <- file.path(DIR_PLOTS, paste0("results_AML_sim_comparisons_CellCnn_main_performance_", cond_names[j], ".pdf"))
