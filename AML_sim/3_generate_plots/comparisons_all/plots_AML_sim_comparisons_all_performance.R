...
# path to save plots
DIR_PLOTS <- "../../../../plots/AML_sim/comparisons_all"
...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    # color scheme
    colors <- c("darkblue", "dodgerblue", "darkslategray3", "gold2", "darkorange1", "brown4")
    ...
    p_ROC <- 
      ...
      ggtitle(paste0("AML-sim: performance comparisons, ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th])), subtitle = "ROC curve") + 
      theme_bw() + 
    ...
    p_TPRFDR <- 
      ...
      ggtitle(paste0("AML-sim: performance comparisons, ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th])), subtitle = "TPR vs. FDR") + 
      theme_bw() + 
      theme(strip.text.x = element_blank()) + 
    ...
    p_TPR <- 
      ...
      ggtitle(paste0("AML-sim: performance comparisons, ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th])), subtitle = "TPR") + 
      theme_bw() + 
      theme(strip.text.x = element_blank(), 
    ...
    p_FPR <- 
      ...
      #coord_fixed() + 
      xlab("False positive rate") + 
      ggtitle(paste0("AML-sim: performance comparisons, ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th])), subtitle = "FPR") + 
      theme_bw() + 
      theme(strip.text.x = element_blank(), 
            axis.text.y = element_blank()) + 
      guides(shape = guide_legend("FDR threshold", override.aes = list(size = 4), order = 1), 
    ...
    ##################################
    # Multi-panel plots: 2 panels only
    ##################################
...
plots_ROC <- lapply(plots_ROC, function(p) {
  p + 
    labs(title = gsub("^.*comparisons, ", "", p$labels$title)) + 
    theme(legend.position = "none")
})
...
# save plots
fn_ROC <- file.path(DIR_PLOTS, paste0("results_AML_sim_comparisons_all_main_performance_ROC.pdf"))
...
# save plots
fn_TPRFDR <- file.path(DIR_PLOTS, paste0("results_AML_sim_comparisons_all_main_performance_TPRFDR.pdf"))
