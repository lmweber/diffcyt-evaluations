...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    fn <- file.path(DIR_PLOTS, "panels", 
                    paste0("results_AML_sim_diffcyt_DA_GLMM_main_MST_", thresholds[th], "_", cond_names[j], ".pdf"))
...
# save plots
fn_MST <- file.path(DIR_PLOTS, "results_AML_sim_diffcyt_DA_GLMM_main_MST.pdf")
