...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    p <- p + 
      ggtitle(paste0("AML-sim, main results: diffcyt-DA-edgeR: ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th]), ": MST"))
    
    fn <- file.path(DIR_PLOTS, "panels", 
                    paste0("results_AML_sim_diffcyt_DA_edgeR_main_MST_", thresholds[th], "_", cond_names[j], ".pdf"))
...
# add combined title
title_MST <- ggdraw() + draw_label("AML-sim, main results: diffcyt-DA-edgeR: MST", fontface = "bold")
...
# save plots
fn_MST <- file.path(DIR_PLOTS, "results_AML_sim_diffcyt_DA_edgeR_main_MST.pdf")
