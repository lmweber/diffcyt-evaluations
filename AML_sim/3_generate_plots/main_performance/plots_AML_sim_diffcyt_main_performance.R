...
# path to save plots
DIR_PLOTS <- "../../../../plots/AML_sim/main_performance"
...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    p_ROC <- 
      ...
      ggtitle(paste0("AML-sim: main results, ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th])), subtitle = "ROC curve") + 
      theme_bw() + 
      theme(strip.text.x = element_blank()) + 
      guides(color = guide_legend("method"))
    
    # save individual panel plot
    fn <- file.path(DIR_PLOTS, "panels", paste0("results_AML_sim_diffcyt_main_ROC_", thresholds[th], "_", cond_names[j], ".pdf"))
    ...
    p_TPRFDR <- 
      ...
      ggtitle(paste0("AML-sim: main results, ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th])), subtitle = "TPR vs. FDR") + 
      theme_bw() + 
      theme(strip.text.x = element_blank()) + 
    ...
    # save individual panel plot
    fn <- file.path(DIR_PLOTS, "panels", paste0("results_AML_sim_diffcyt_main_TPRFDR_", thresholds[th], "_", cond_names[j], ".pdf"))
    ggsave(fn, width = 4.75, height = 3.5)
    ...
    p_TPR <- 
      ...
      ggtitle(paste0("AML-sim: main results, ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th])), subtitle = "TPR") + 
      theme_bw() + 
      theme(strip.text.x = element_blank(), 
    ...
    # save individual panel plot
    fn <- file.path(DIR_PLOTS, "panels", paste0("results_AML_sim_diffcyt_main_TPR_", thresholds[th], "_", cond_names[j], ".pdf"))
    ggsave(fn, width = 4.75, height = 3.5)
    ...
    p_FPR <- 
      ...
      #coord_fixed() + 
      xlab("False positive rate") + 
      ggtitle(paste0("AML-sim: main results, ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th])), subtitle = "FPR") + 
      theme_bw() + 
      theme(strip.text.x = element_blank(), 
            axis.text.y = element_blank()) + 
      guides(shape = guide_legend("FDR threshold", override.aes = list(size = 4), order = 1), 
    ...
    # save individual panel plot
    fn <- file.path(DIR_PLOTS, "panels", paste0("results_AML_sim_diffcyt_main_FPR_", thresholds[th], "_", cond_names[j], ".pdf"))
    ggsave(fn, width = 4.75, height = 3.5)
    ...
    plots_list <- lapply(plots_list, function(p) {
      p + 
        labs(title = p$labels$subtitle, subtitle = gsub("^.*results, ", "", p$labels$title)) + 
    ...
    fn <- file.path(DIR_PLOTS, paste0("results_AML_sim_diffcyt_main_performance_", thresholds[th], "_", cond_names[j], ".pdf"))
...
for (j in 1:length(cond_names)) {
  ...
  # save combined plot
  fn <- file.path(DIR_PLOTS, paste0("results_AML_sim_diffcyt_main_performance_", cond_names[j], ".pdf"))
