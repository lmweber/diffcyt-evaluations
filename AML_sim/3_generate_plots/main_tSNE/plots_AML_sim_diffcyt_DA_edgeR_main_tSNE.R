...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    p <- p + 
      ggtitle(paste0("AML-sim, main results: diffcyt-DA-edgeR: ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th]), ": tSNE"))
...
# add combined title
title_tSNE <- ggdraw() + draw_label("AML-sim, main results: diffcyt-DA-edgeR: tSNE", fontface = "bold")
