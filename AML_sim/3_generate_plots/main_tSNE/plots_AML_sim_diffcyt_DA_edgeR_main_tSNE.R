...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    p <- p + 
      ggtitle(paste0("AML-sim, main results: diffcyt-DA-edgeR: ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th]), ": t-SNE"))
    
    fn <- file.path(DIR_PLOTS, "panels", 
                    paste0("results_AML_sim_diffcyt_DA_edgeR_main_tSNE_", thresholds[th], "_", cond_names[j], ".pdf"))
...
# add combined title
title_tSNE <- ggdraw() + draw_label("AML-sim, main results: diffcyt-DA-edgeR: t-SNE", fontface = "bold")
...
# save plots
fn_tSNE <- file.path(DIR_PLOTS, "results_AML_sim_diffcyt_DA_edgeR_main_tSNE.pdf")
