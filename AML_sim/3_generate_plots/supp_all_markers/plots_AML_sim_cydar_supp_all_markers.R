...
load(file.path(DIR_RDATA_SUPP_ALL_MARKERS, "outputs_AML_sim_cydar_supp_all_markers.RData"))
...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    data <- list(lineage_markers = out_cydar_main[[th]][[j]], 
                 all_markers = out_cydar_supp_all_markers[[th]][[j]])
    ...
    p <- p + 
      ggtitle(paste0("AML-sim, all markers: cydar: ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th]), ": ROC curves"))
    
    fn <- file.path(DIR_PLOTS, "panels", 
                    paste0("results_AML_sim_cydar_supp_all_markers_ROC_curves_", thresholds[th], "_", cond_names[j], ".pdf"))
...
# add combined title
title_ROC <- ggdraw() + draw_label("AML-sim, all markers: cydar: ROC curves", fontface = "bold")
...
# save plots
fn_ROC <- file.path(DIR_PLOTS, "results_AML_sim_cydar_supp_all_markers_ROC_curves.pdf")
