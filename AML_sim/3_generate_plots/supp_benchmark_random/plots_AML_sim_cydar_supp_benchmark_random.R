...
load(file.path(DIR_RDATA_MAIN, "outputs_AML_sim_cydar_main.RData"))
load(file.path(DIR_RDATA_SUPP_BENCHMARK_RANDOM_SEED1, "outputs_AML_sim_cydar_supp_benchmark_random_seed1.RData"))
load(file.path(DIR_RDATA_SUPP_BENCHMARK_RANDOM_SEED2, "outputs_AML_sim_cydar_supp_benchmark_random_seed2.RData"))
load(file.path(DIR_RDATA_SUPP_BENCHMARK_RANDOM_SEED3, "outputs_AML_sim_cydar_supp_benchmark_random_seed3.RData"))
...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    data <- list(main = out_cydar_main[[th]][[j]], 
                 seed1 = out_cydar_supp_benchmark_random_seed1[[th]][[j]], 
                 seed2 = out_cydar_supp_benchmark_random_seed2[[th]][[j]], 
                 seed3 = out_cydar_supp_benchmark_random_seed3[[th]][[j]])
    ...
    cobradata <- COBRAData(pval = data.frame(main = data[["main"]][, "p_vals"], 
                           ...
                           padj = data.frame(main = data[["main"]][, "q_vals"], 
                                             seed1 = data[["seed1"]][, "q_vals"], 
                                             seed2 = data[["seed2"]][, "q_vals"], 
                                             seed3 = data[["seed3"]][, "q_vals"]), 
    ...
    colors <- c("salmon", "salmon", "salmon", "salmon")
    ...
    p <- p + 
      ggtitle(paste0("AML-sim, random benchmark data: cydar: ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th]), ": ROC curves"))
    
    fn <- file.path(DIR_PLOTS, "panels", 
                    paste0("results_AML_sim_cydar_supp_benchmark_random_ROC_curves_", thresholds[th], "_", cond_names[j], ".pdf"))
...
# add combined title
title_ROC <- ggdraw() + draw_label("AML-sim, randomized benchmark data: cydar: ROC curves", fontface = "bold")
...
# save plots
fn_ROC <- file.path(DIR_PLOTS, "results_AML_sim_cydar_supp_benchmark_random_ROC_curves.pdf")
