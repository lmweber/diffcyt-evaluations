...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    fn <- file.path(DIR_PLOTS, "panels", 
                    paste0("results_AML_sim_diffcyt_DA_limma_supp_clustering_resolution_ROC_curves_", thresholds[th], "_", cond_names[j], ".pdf"))
...
for (j in 1:length(cond_names)) {
  ...
  # save individual panel plot
  p <- p + 
    ggtitle(paste0("AML-sim, clustering resolution: diffcyt-DA-limma: ", cond_names[j], ": pAUC (FPR < ", thresh, ")"))
  
  fn <- file.path(DIR_PLOTS, "panels", 
                  paste0("results_AML_sim_diffcyt_DA_limma_supp_clustering_resolution_pAUC_", cond_names[j], ".pdf"))
  ggsave(fn, width = 7, height = 5.5)
...
# save plots
fn_ROC <- file.path(DIR_PLOTS, "results_AML_sim_diffcyt_DA_limma_supp_clustering_resolution_ROC_curves.pdf")
...
# save plots
fn_pAUC <- file.path(DIR_PLOTS, "results_AML_sim_diffcyt_DA_limma_supp_clustering_resolution_pAUC.pdf")
