...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    p <- p + 
      ggtitle(paste0("diffcyt-DA-limma: AML-sim, clustering resolution, ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th]), ": ROC curves"))
...
# add combined title
title_ROC <- ggdraw() + draw_label("diffcyt-DA-limma: AML-sim, clustering resolution: ROC curves", fontface = "bold")
