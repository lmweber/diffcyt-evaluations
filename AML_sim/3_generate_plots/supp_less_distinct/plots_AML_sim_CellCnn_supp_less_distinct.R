...
load(file.path(DIR_RDATA_SUPP_LESS_DISTINCT_50, "outputs_AML_sim_CellCnn_supp_less_distinct_50.RData"))
load(file.path(DIR_RDATA_SUPP_LESS_DISTINCT_75, "outputs_AML_sim_CellCnn_supp_less_distinct_75.RData"))
...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    data <- list(main = out_CellCnn_main[[th]][[j]], 
                 less_50pc = out_CellCnn_supp_less_distinct_50[[th]][[j]], 
                 less_75pc = out_CellCnn_supp_less_distinct_75[[th]][[j]])
    ...
    cobradata <- COBRAData(score = data.frame(main = data[["main"]][, "scores"], 
                                              less_50pc = data[["less_50pc"]][, "scores"], 
                                              less_75pc = data[["less_75pc"]][, "scores"]), 
    ...
    p <- p + 
      ggtitle(paste0("AML-sim, 'less distinct' benchmark data: CellCnn: ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th]), ": ROC curves"))
    
    fn <- file.path(DIR_PLOTS, "panels", 
                    paste0("results_AML_sim_CellCnn_supp_less_distinct_ROC_curves_", thresholds[th], "_", cond_names[j], ".pdf"))
...
# add combined title
title_ROC <- ggdraw() + draw_label("AML-sim, 'less distinct' benchmark data: CellCnn: ROC curves", fontface = "bold")
...
# save plots
fn_ROC <- file.path(DIR_PLOTS, "results_AML_sim_CellCnn_supp_less_distinct_ROC_curves.pdf")
