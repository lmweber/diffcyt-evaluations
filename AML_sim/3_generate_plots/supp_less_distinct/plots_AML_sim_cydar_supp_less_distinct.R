...
load(file.path(DIR_RDATA_SUPP_LESS_DISTINCT_50, "outputs_AML_sim_cydar_supp_less_distinct_50.RData"))
load(file.path(DIR_RDATA_SUPP_LESS_DISTINCT_75, "outputs_AML_sim_cydar_supp_less_distinct_75.RData"))
...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    data <- list(main = out_cydar_main[[th]][[j]], 
                 less_50pc = out_cydar_supp_less_distinct_50[[th]][[j]], 
                 less_75pc = out_cydar_supp_less_distinct_75[[th]][[j]])
    ...
    cobradata <- COBRAData(pval = data.frame(main = data[["main"]][, "p_vals"], 
                           ...
                           padj = data.frame(main = data[["main"]][, "q_vals"], 
                                             less_50pc = data[["less_50pc"]][, "q_vals"], 
                                             less_75pc = data[["less_75pc"]][, "q_vals"]), 
    ...
    p <- p + 
      ggtitle(paste0("AML-sim, 'less distinct' benchmark data: cydar: ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th]), ": ROC curves"))
    
    fn <- file.path(DIR_PLOTS, "panels", 
                    paste0("results_AML_sim_cydar_supp_less_distinct_ROC_curves_", thresholds[th], "_", cond_names[j], ".pdf"))
...
# add combined title
title_ROC <- ggdraw() + draw_label("AML-sim, 'less distinct' benchmark data: cydar: ROC curves", fontface = "bold")
...
# save plots
fn_ROC <- file.path(DIR_PLOTS, "results_AML_sim_cydar_supp_less_distinct_ROC_curves.pdf")
