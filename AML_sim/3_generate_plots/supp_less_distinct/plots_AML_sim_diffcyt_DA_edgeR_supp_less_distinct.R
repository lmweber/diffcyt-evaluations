...
load(file.path(DIR_RDATA_SUPP_LESS_DISTINCT, "outputs_AML_sim_diffcyt_DA_edgeR_supp_less_distinct.RData"))
...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    data <- list(main = out_diffcyt_DA_edgeR_main[[th]][[j]], 
                 less_50pc = out_diffcyt_DA_edgeR_supp_less_distinct[["less_50pc"]][[th]][[j]], 
                 less_75pc = out_diffcyt_DA_edgeR_supp_less_distinct[["less_75pc"]][[th]][[j]])
    ...
    fn <- file.path(DIR_PLOTS, "panels", 
                    paste0("results_AML_sim_diffcyt_DA_edgeR_supp_less_distinct_ROC_", thresholds[th], "_", cond_names[j], ".pdf"))
...
# save plots
fn_multi <- file.path(DIR_PLOTS, paste0("results_AML_sim_diffcyt_DA_edgeR_supp_less_distinct_ROC.pdf"))
