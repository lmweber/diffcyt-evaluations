...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    cobradata <- COBRAData(pval = data.frame(diffcyt_DA_voom = data[["diffcyt_DA_voom"]][, "p_val"]), 
    ...
    p_ROC <- 
      ...
      ggtitle(paste0("AML-sim: random effects, ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th])), subtitle = "ROC curve") + 
      theme_bw() + 
    ...
    p_TPRFDR <- 
      ...
      ggtitle(paste0("AML-sim: random effects, ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th])), subtitle = "TPR vs. FDR") + 
      theme_bw() + 
      theme(strip.text.x = element_blank()) + 
    ...
    p_TPR <- 
      ...
      ggtitle(paste0("AML-sim: random effects, ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th])), subtitle = "TPR") + 
      theme_bw() + 
      theme(strip.text.x = element_blank(), 
    ...
    p_FPR <- 
      ...
      #coord_fixed() + 
      xlab("False positive rate") + 
      ggtitle(paste0("AML-sim: random effects, ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th])), subtitle = "FPR") + 
      theme_bw() + 
      theme(strip.text.x = element_blank(), 
            axis.text.y = element_blank()) + 
      guides(shape = guide_legend("FDR threshold", override.aes = list(size = 4), order = 1), 
    ...
    plots_list <- lapply(plots_list, function(p) {
      p + 
        labs(title = p$labels$subtitle, subtitle = gsub("^.*effects, ", "", p$labels$title)) + 
