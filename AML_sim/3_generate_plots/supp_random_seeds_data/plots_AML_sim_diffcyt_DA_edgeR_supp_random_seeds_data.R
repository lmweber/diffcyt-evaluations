...
load(file.path(DIR_RDATA_SUPP_RANDOM_SEEDS_DATA, "outputs_AML_sim_diffcyt_DA_edgeR_supp_random_seeds_data.RData"))


# path to save plots
DIR_PLOTS <- "../../../../plots/AML_sim/supp_random_seeds_data"
...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    data <- list(main = out_diffcyt_DA_edgeR_main[[th]][[j]], 
                 seed_1 = out_diffcyt_DA_edgeR_supp_random_seeds_data[[1]][[th]][[j]], 
                 seed_2 = out_diffcyt_DA_edgeR_supp_random_seeds_data[[2]][[th]][[j]], 
                 seed_3 = out_diffcyt_DA_edgeR_supp_random_seeds_data[[3]][[th]][[j]])
    ...
    p_ROC <- 
      ...
      ggtitle(paste0("AML-sim: random seeds for data generation, ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th])), subtitle = "ROC curve") + 
      theme_bw() + 
    ...
    d_plot$method_names <- as.factor("diffcyt-DA-edgeR")
    ...
    fn <- file.path(DIR_PLOTS, "panels", 
                    paste0("results_AML_sim_diffcyt_DA_edgeR_supp_random_seeds_data_ROC_", thresholds[th], "_", cond_names[j], ".pdf"))
...
plots_multi <- lapply(plots_all, function(p) {
  p + 
    labs(title = gsub("^.*generation, ", "", p$labels$title)) + 
...
# save plots
fn_multi <- file.path(DIR_PLOTS, paste0("results_AML_sim_diffcyt_DA_edgeR_supp_random_seeds_data_ROC.pdf"))
