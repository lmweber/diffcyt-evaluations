...
load(file.path(DIR_RDATA_SUPP_RANDOM_STARTS, "outputs_AML_sim_diffcyt_DA_GLMM_supp_random_starts.RData"))
...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    data <- list(seed_main = out_diffcyt_DA_GLMM_main[[th]][[j]], 
                 seed_alt_1 = out_diffcyt_DA_GLMM_supp_random_starts[[1]][[th]][[j]], 
                 seed_alt_2 = out_diffcyt_DA_GLMM_supp_random_starts[[2]][[th]][[j]], 
                 seed_alt_3 = out_diffcyt_DA_GLMM_supp_random_starts[[3]][[th]][[j]])
    ...
    p <- p + 
      ggtitle(paste0("AML-sim, random starts: diffcyt-DA-GLMM: ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th]), ": ROC curves"))
    
    fn <- file.path(DIR_PLOTS, "panels", 
                    paste0("results_AML_sim_diffcyt_DA_GLMM_supp_random_starts_ROC_curves_", thresholds[th], "_", cond_names[j], ".pdf"))
...
# add combined title
title_ROC <- ggdraw() + draw_label("AML-sim, random starts for clustering: diffcyt-DA-GLMM: ROC curves", fontface = "bold")
...
# save plots
fn_ROC <- file.path(DIR_PLOTS, "results_AML_sim_diffcyt_DA_GLMM_supp_random_starts_ROC_curves.pdf")
