...
# path to save plots
DIR_PLOTS <- "../../../../plots/AML_sim/supp_sample_sizes"
...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    p_ROC <- 
      ...
      ggtitle(paste0("AML-sim: smaller sample sizes: 2 vs. 2, ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th])), subtitle = "ROC curve") + 
      theme_bw() + 
    ...
    p_TPRFDR <- 
      ...
      ggtitle(paste0("AML-sim: smaller sample sizes: 2 vs. 2, ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th])), subtitle = "TPR vs. FDR") + 
      theme_bw() + 
      theme(strip.text.x = element_blank()) + 
    ...
    p_TPR <- 
      ...
      ggtitle(paste0("AML-sim: smaller sample sizes: 2 vs. 2, ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th])), subtitle = "TPR") + 
      theme_bw() + 
      theme(strip.text.x = element_blank(), 
    ...
    # save individual panel plot
    fn <- file.path(DIR_PLOTS, "panels", paste0("results_AML_sim_diffcyt_supp_sample_sizes_TPR_", thresholds[th], "_", cond_names[j], ".pdf"))
    ggsave(fn, width = 4.75, height = 3.5)
    ...
    p_FPR <- 
      ...
      #coord_fixed() + 
      xlab("False positive rate") + 
      ggtitle(paste0("AML-sim: smaller sample sizes: 2 vs. 2, ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th])), subtitle = "FPR") + 
      theme_bw() + 
      theme(strip.text.x = element_blank(), 
            axis.text.y = element_blank()) + 
      guides(shape = guide_legend("FDR threshold", override.aes = list(size = 4), order = 1), 
    ...
    # save individual panel plot
    fn <- file.path(DIR_PLOTS, "panels", paste0("results_AML_sim_diffcyt_supp_sample_sizes_FPR_", thresholds[th], "_", cond_names[j], ".pdf"))
    ggsave(fn, width = 4.75, height = 3.5)
    ...
    plots_list <- lapply(plots_list, function(p) {
      p + 
        labs(title = p$labels$subtitle, subtitle = gsub("^.*2, ", "", p$labels$title)) + 
    ...
    fn <- file.path(DIR_PLOTS, paste0("results_AML_sim_diffcyt_supp_sample_sizes_", thresholds[th], "_", cond_names[j], ".pdf"))
...
for (j in 1:length(cond_names)) {
  ...
  # save combined plot
  fn <- file.path(DIR_PLOTS, paste0("results_AML_sim_diffcyt_supp_sample_sizes_", cond_names[j], ".pdf"))
