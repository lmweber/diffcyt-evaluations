...
load(file.path(DIR_RDATA_SUPP_SEPARATE_CLUSTERING, "outputs_AML_sim_diffcyt_DA_limma_supp_separate_clustering.RData"))
...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    data <- list(combined_clustering = out_diffcyt_DA_limma_main[[th]][[j]], 
                 separate_clustering = out_diffcyt_DA_limma_supp_separate_clustering[[th]][[j]])
    ...
    p <- p + 
      ggtitle(paste0("AML-sim, separate clustering: diffcyt-DA-limma: ", cond_names[j], ", ", gsub("pc$", "\\%", thresholds[th]), ": ROC curves"))
    
    fn <- file.path(DIR_PLOTS, "panels", 
                    paste0("results_AML_sim_diffcyt_DA_limma_supp_separate_clustering_ROC_curves_", thresholds[th], "_", cond_names[j], ".pdf"))
...
# add combined title
title_ROC <- ggdraw() + draw_label("AML-sim, separate clustering: diffcyt-DA-limma: ROC curves", fontface = "bold")
...
# save plots
fn_ROC <- file.path(DIR_PLOTS, "results_AML_sim_diffcyt_DA_limma_supp_separate_clustering_ROC_curves.pdf")
