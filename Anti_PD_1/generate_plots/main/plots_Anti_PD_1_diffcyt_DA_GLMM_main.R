...
d_medians_all <- out_objects_diffcyt_DA_GLMM_main$d_medians_all
...
ht_main_title <- "Anti-PD-1, diffcyt-DA-GLMM: phenotype of detected clusters"


# save plot
fn <- file.path(DIR_PLOTS, "results_Anti_PD_1_diffcyt_DA_GLMM_main_heatmap_phenotype.pdf")
...
ht_abundance_title <- "Anti-PD-1, diffcyt-DA-GLMM: abundance of detected clusters"
...
ggplot(d_boxplots, aes(x = group, y = percent, color = group)) + 
  ...
  geom_point() + 
  labs(title = "Anti-PD-1, diffcyt-DA-GLMM: combined abundance") + 
  theme_bw()

fn <- file.path(DIR_PLOTS, "results_Anti_PD_1_diffcyt_DA_GLMM_main_boxplots.pdf")
