...
DIR_SESSION_INFO <- "../../../../session_info/Anti_PD_1/supp_sensitivity"
...
out_diffcyt_DA_GLMM_supp_sensitivity <- 
  out_clusters_diffcyt_DA_GLMM_supp_sensitivity <- 
  out_objects_diffcyt_DA_GLMM_supp_sensitivity <- 
  runtime_diffcyt_DA_GLMM_supp_sensitivity <- vector("list", length(seeds))
names(out_diffcyt_DA_GLMM_supp_sensitivity) <- 
  names(out_clusters_diffcyt_DA_GLMM_supp_sensitivity) <- 
  names(out_objects_diffcyt_DA_GLMM_supp_sensitivity) <- 
  names(runtime_diffcyt_DA_GLMM_supp_sensitivity) <- seeds
...
for (s in 1:length(seeds)) {
  ...
  # note: using different random seeds
  ...
  runtime_preprocessing <- system.time({
    ...
    # note: using different random seeds
    d_se <- generateClusters(d_se, xdim = 20, ydim = 20, seed_clustering = seeds[s])
  ...
  out_objects_diffcyt_DA_GLMM_supp_sensitivity[[s]] <- list(
    d_se = d_se, 
    d_counts = d_counts, 
    d_medians = d_medians, 
  ...
  runtime_diffcyt_DA_GLMM_supp_sensitivity[[s]] <- runtime_total
  ...
  out_clusters_diffcyt_DA_GLMM_supp_sensitivity[[s]] <- res_clusters
...
save(runtime_diffcyt_DA_GLMM_supp_sensitivity, 
     file = file.path(DIR_RDATA, "outputs_Anti_PD_1_diffcyt_DA_GLMM_supp_sensitivity.RData"))

save(out_clusters_diffcyt_DA_GLMM_supp_sensitivity, 
     file = file.path(DIR_RDATA, "out_clusters_Anti_PD_1_diffcyt_DA_GLMM_supp_sensitivity.RData"))

save(out_objects_diffcyt_DA_GLMM_supp_sensitivity, 
     file = file.path(DIR_RDATA, "out_objects_Anti_PD_1_diffcyt_DA_GLMM_supp_sensitivity.RData"))
...
sink(file.path(DIR_SESSION_INFO, "session_info_Anti_PD_1_diffcyt_DA_GLMM_supp_sensitivity.txt"))
sessionInfo()
sink()
