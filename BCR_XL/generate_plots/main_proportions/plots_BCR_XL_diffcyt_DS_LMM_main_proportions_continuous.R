...
load(file.path(DIR_RDATA, "out_clusters_BCR_XL_diffcyt_DS_LMM_main.RData"))
load(file.path(DIR_RDATA, "out_objects_BCR_XL_diffcyt_DS_LMM_main.RData"))
load(file.path(DIR_RDATA, "outputs_BCR_XL_diffcyt_DS_LMM_main.RData"))
...
res <- out_clusters_diffcyt_DS_LMM_main
table(res$p_adj <= 0.1)
dim(res)

n_clus <- nlevels(cluster_IDs)
n_state_markers <- sum(metadata(out_objects_diffcyt_DS_LMM_main$d_medians)$id_state_markers)

# note: showing adjusted p-values directly
d_detected_markers <- matrix(1, nrow = n_clus, ncol = n_state_markers)
...
ht_title <- "BCR-XL, diffcyt-DS-LMM: cluster proportions and detected markers"

fn <- file.path(DIR_PLOTS, "diffcyt_DS_LMM", "results_BCR_XL_diffcyt_DS_LMM_main_proportions_continuous.pdf")
