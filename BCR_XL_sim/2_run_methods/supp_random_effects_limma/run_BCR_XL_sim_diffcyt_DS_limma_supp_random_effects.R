...
# note: include random effects for 'patient_id' (using 'duplicateCorrelation' methodology)
contrast_vec <- c(0, 1)
...
runtime_tests <- system.time({
  ...
  # note: include random effects for 'patient_id' (using 'duplicateCorrelation' methodology)
  design <- createDesignMatrix(experiment_info, cols_design = 1)
  design
  ...
  # run tests
  # note: include random effects for 'patient_id' (using 'duplicateCorrelation' methodology)
  res <- testDS_limma(d_counts, d_medians, design, contrast, block = patient_id, path = DIR_PLOTS)
