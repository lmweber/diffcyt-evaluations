...
load(file.path(DIR_RDATA_SUPP_LESS_DISTINCT, "outputs_BCR_XL_sim_diffcyt_DS_LMM_supp_less_distinct.RData"))
...
data_less_50pc <- out_diffcyt_DS_LMM_supp_less_distinct[["less_50pc"]]
data_less_75pc <- out_diffcyt_DS_LMM_supp_less_distinct[["less_75pc"]]
...
for (i in 1:length(cobraplot_list)) {
  ...
  # save plot
  fn <- file.path(DIR_PLOTS, "panels", paste0("results_BCR_XL_sim_diffcyt_DS_LMM_supp_less_distinct_ROC_", distinctness_names[i], ".pdf"))
...
# save plot
fn <- file.path(DIR_PLOTS, "results_BCR_XL_sim_diffcyt_DS_LMM_supp_less_distinct_ROC.pdf")
ggsave(fn, width = 4.75, height = 3.5)
