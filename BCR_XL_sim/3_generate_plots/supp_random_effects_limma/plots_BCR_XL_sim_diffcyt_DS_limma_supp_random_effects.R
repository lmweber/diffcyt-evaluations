...
DIR_RDATA_RANDOM_EFFECTS_LIMMA <- "../../../../RData/BCR_XL_sim/supp_random_effects_limma"

load(file.path(DIR_RDATA_MAIN, "outputs_BCR_XL_sim_diffcyt_DS_limma_main.RData"))
load(file.path(DIR_RDATA_RANDOM_EFFECTS_LIMMA, "outputs_BCR_XL_sim_diffcyt_DS_limma_supp_random_effects.RData"))
...
data <- list(main = out_diffcyt_DS_limma_main, 
             random_effects = out_diffcyt_DS_limma_supp_random_effects)
...
p_ROC <- 
  ...
  ggtitle("BCR-XL-sim: diffcyt-DS-limma", subtitle = "ROC curve") + 
  theme_bw() + 
  theme(strip.text.x = element_blank()) + 
  guides(color = guide_legend("method"), linetype = guide_legend("method"))

# save plot
fn <- file.path(DIR_PLOTS, "results_BCR_XL_sim_diffcyt_DS_limma_supp_random_effects_ROC.pdf")
...
p_TPRFDR <- 
  ...
  ggtitle("BCR-XL-sim: diffcyt-DS-limma", subtitle = "TPR vs. FDR") + 
  theme_bw() + 
  theme(strip.text.x = element_blank()) + 
...
# save plot
fn <- file.path(DIR_PLOTS, "results_BCR_XL_sim_diffcyt_DS_limma_supp_random_effects_TPRFDR.pdf")
ggsave(fn, width = 4.25, height = 3)
