...
load(file.path(DIR_RDATA_SUPP_RANDOM_SEEDS_DATA, "outputs_BCR_XL_sim_diffcyt_DS_LMM_supp_random_seeds_data.RData"))
...
data_main   <- out_diffcyt_DS_LMM_main
data_seed_1 <- out_diffcyt_DS_LMM_supp_random_seeds_data[[1]]
...
for (i in 1:length(cobraplot_list)) {
  ...
  # save plot
  fn <- file.path(DIR_PLOTS, "panels", paste0("results_BCR_XL_sim_diffcyt_DS_LMM_supp_random_seeds_clustering_ROC_", seed_names[i], ".pdf"))
...
d_combined$method_names <- as.factor("diffcyt-DS-LMM")
...
# save plot
fn <- file.path(DIR_PLOTS, "results_BCR_XL_sim_diffcyt_DS_LMM_supp_random_seeds_data_ROC.pdf")
ggsave(fn, width = 4.75, height = 3.5)
