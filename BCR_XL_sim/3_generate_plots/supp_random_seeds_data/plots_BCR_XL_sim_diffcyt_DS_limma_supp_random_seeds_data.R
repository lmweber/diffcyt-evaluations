...
load(file.path(DIR_RDATA_SUPP_RANDOM_SEEDS_DATA, "outputs_BCR_XL_sim_diffcyt_DS_limma_supp_random_seeds_data.RData"))
...
data_seed_1 <- out_diffcyt_DS_limma_supp_random_seeds_data[[1]]
data_seed_2 <- out_diffcyt_DS_limma_supp_random_seeds_data[[2]]
data_seed_3 <- out_diffcyt_DS_limma_supp_random_seeds_data[[3]]
...
cobraplot_seed_1 <- prepare_data_for_plot(cobraperf_seed_1, colorscheme = colors[1])
cobraplot_seed_2 <- prepare_data_for_plot(cobraperf_seed_2, colorscheme = colors[1])
cobraplot_seed_3 <- prepare_data_for_plot(cobraperf_seed_3, colorscheme = colors[1])
...
for (i in 1:length(cobraplot_list)) {
  ...
  # save plot
  fn <- file.path(DIR_PLOTS, "panels", paste0("results_BCR_XL_sim_diffcyt_DS_limma_supp_random_seeds_clustering_ROC_", seed_names[i], ".pdf"))
...
d_combined$seed_names <- factor(d_combined$method, levels = seed_names)
d_combined$method_names <- as.factor("diffcyt-DS-limma")

# create plot
p_combined <- 
  ggplot(d_combined, aes(x = FPR, y = TPR, linetype = seed_names, color = method_names)) + 
  geom_line(lwd = 0.75) + 
...
# save plot
fn <- file.path(DIR_PLOTS, "results_BCR_XL_sim_diffcyt_DS_limma_supp_random_seeds_data_ROC.pdf")
ggsave(fn, width = 4.75, height = 3.5)
