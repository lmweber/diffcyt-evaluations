...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    data <- out_Citrus_lineage_markers[[th]][[j]]
    cobradata <- COBRAData(score = data.frame(Citrus_lineage_markers = data[, "scores"]), 
    ...
    path <- paste0("../../../plots/AML_spike_in/Citrus/lineage_markers/", thresholds[th], "/", cond_names[j])
    filename <- file.path(path, paste0("results_Citrus_lineage_markers_ROC_curve_", thresholds[th], "_", cond_names[j], ".pdf"))
