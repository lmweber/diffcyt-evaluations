...
for (th in 1:length(thresholds)) {
  ...
  for (j in 1:length(cond_names)) {
    ...
    for (i in 1:length(sample_IDs_keep)) {
      path <- paste0(DIR_CELLCNN_FILES, "/data_transformed/AML_spike_in/all_markers/", thresholds[th], "/", cond_names[j])
    ...
    fn_samples <- paste0(DIR_CELLCNN_FILES, "/inputs/AML_spike_in/all_markers/", thresholds[th], "/", cond_names[j], "/input_samples.csv")
    write.csv(df_samples, fn_samples, quote = FALSE, row.names = FALSE)
    
    # need to use 'write.table' to allow removing column names
    fn_markers <- paste0(DIR_CELLCNN_FILES, "/inputs/AML_spike_in/all_markers/", thresholds[th], "/", cond_names[j], "/input_markers.csv")
    ...
    cmd <- paste("python", paste0(DIR_CELLCNN, "/cellCnn/run_analysis.py"), 
                 paste0("-f ", DIR_CELLCNN_FILES, "/inputs/AML_spike_in/all_markers/", thresholds[th], "/", cond_names[j], "/input_samples.csv"), 
                 paste0("-m ", DIR_CELLCNN_FILES, "/inputs/AML_spike_in/all_markers/", thresholds[th], "/", cond_names[j], "/input_markers.csv"), 
                 paste0("-i ", DIR_CELLCNN_FILES, "/data_transformed/AML_spike_in/all_markers/", thresholds[th], "/", cond_names[j], "/"), 
                 paste0("-o ", DIR_CELLCNN_FILES, "/out_CellCnn/AML_spike_in/all_markers/", thresholds[th], "/", cond_names[j], "/"), 
                 "--export_csv", 
    ...
    sink(paste0(DIR_CELLCNN_FILES, "/runtime/AML_spike_in/all_markers/", thresholds[th], "/", cond_names[j], "/runtime_analysis.txt"))
    runtime_analysis
    sink()
    ...
    sink(paste0(DIR_CELLCNN_FILES, "/runtime/AML_spike_in/all_markers/", thresholds[th], "/", cond_names[j], "/runtime_select.txt"))
    runtime_select
    sink()
    ...
    path_out <- paste0(DIR_CELLCNN_FILES, "/out_CellCnn/AML_spike_in/all_markers/", thresholds[th], "/", cond_names[j], "/selected_cells")
...
save(out_CellCnn_all_markers, file = file.path(DIR_RDATA, "/outputs_AML_spike_in_CellCnn_all_markers.RData"))
...
sink(file.path(DIR_SESSION_INFO, "/session_info_AML_spike_in_CellCnn_all_markers.txt"))
sessionInfo()
sink()
