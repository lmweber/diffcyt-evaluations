...
out_cydar_all_markers <- vector("list", length(thresholds))
names(out_cydar_all_markers) <- thresholds
...
for (th in 1:length(thresholds)) {
  ...
  out_cydar_all_markers[[th]] <- vector("list", length(cond_names))
  names(out_cydar_all_markers[[th]]) <- cond_names
  ...
  for (j in 1:length(cond_names)) {
    ...
    # store results
    out_cydar_all_markers[[th]][[j]] <- res
