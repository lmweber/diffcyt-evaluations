...
for (th in 1:length(thresholds)) {
  ...
  # modify to use all markers
  cols_to_use <- colData(d_se)$is_marker_col
  
  seed <- 123
  runtime_clustering <- system.time(
    d_se <- generateClusters(d_se, cols_to_use = cols_to_use, xdim = 30, ydim = 30, seed = seed)
  )
...
save.image("../../../RData/AML_spike_in/outputs_AML_spike_in_diffcyt_DA_limma_all_markers.RData")
