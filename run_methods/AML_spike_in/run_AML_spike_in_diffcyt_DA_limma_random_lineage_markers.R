...
# 
# - method: diffcyt-DA-limma-random-lineage-markers
...
out_diffcyt_DA_limma_random_lineage_markers <- vector("list", length(thresholds))
names(out_diffcyt_DA_limma_random_lineage_markers) <- thresholds
...
for (th in 1:length(thresholds)) {
  ...
  out_diffcyt_DA_limma_random_lineage_markers[[th]] <- vector("list", length(cond_names))
  names(out_diffcyt_DA_limma_random_lineage_markers[[th]]) <- cond_names
  ...
  for (j in 1:length(cond_names)) {
    ...
    path <- paste0("../../../plots/AML_spike_in/diffcyt_DA_limma_random/lineage_markers/", thresholds[th], "/", cond_names[j])
    runtime <- system.time(
    ...
    # store results
    out_diffcyt_DA_limma_random_lineage_markers[[th]][[j]] <- res
...
save(out_diffcyt_DA_limma_random_lineage_markers, file = "../../../RData/AML_spike_in/outputs_AML_spike_in_diffcyt_DA_limma_random_lineage_markers.RData")
...
sink("../../../session_info/AML_spike_in/session_info_AML_spike_in_diffcyt_DA_limma_random_lineage_markers.txt")
sessionInfo()
sink()
